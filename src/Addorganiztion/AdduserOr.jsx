...
const AdduserOr = () => {
  ...
  const handleSave = () => {
    ...
    if (edit) {
      axios
        .put(
          `https://backndVoo.voo-hub.com/api/ornization/user/update/${id}`,
          newUser,
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    axios
      .post("https://backndVoo.voo-hub.com/api/ornization/user/add", newUser, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      ...
      <div className="flex flex-wrap gap-6 mt-6">
        ...
        <div className="flex flex-col gap-3 items-start justify-end">
          ...
          <div className="relative w-[200px] md:w-[300px] h-[48px] md:h-[72px]">
            <FaRegCalendarAlt
              className={`absolute top-1/2 transform -translate-y-1/2 text-one ${
                isArabic ? "left-3" : "right-3"
              } text-[20px]  cursor-pointer  z-10`}
            />
            <DatePicker
              selected={birthdate}
