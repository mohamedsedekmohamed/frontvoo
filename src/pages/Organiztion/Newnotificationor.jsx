...
useEffect(() => {
  axios.get("https://backndvoo.voo-hub.com/api/ornization/noti/view", {
    headers: {
    ...
    if (unreadCount > 0) {
      axios.post("https://backndvoo.voo-hub.com/api/ornization/noti/view_notification", {
...
const handleOpenDetails = (notif) => {
  ...
  axios
    .post(
      `https://backndvoo.voo-hub.com/api/ornization/noti/view_request/${notif.id}`,
      {},
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="w-full px-4 py-6" dir="ltr">
      <h1 className="text-one text-4xl text-center font-bold mb-6">{t("Notifications")}</h1>
        ...
        <div className="fixed top-0 left-0 w-full h-full bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl flex items-center gap-4 p-6 w-[90%] max-w-2xl border-[3px] border-one">
            <div className="flex-1">
              <h2 className="text-2xl font-bold mb-2 text-one">{t("NotificationDetails")}</h2>
...
export default Newnotificationor;
