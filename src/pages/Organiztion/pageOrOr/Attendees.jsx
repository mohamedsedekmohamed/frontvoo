...
const Attendees = ({ id }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://backndVoo.voo-hub.com/api/orgnization/getEventVolunteers/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then((response) => {
        const volunteers = response.data.users?.event_volunteers || [];
        setData(volunteers);
      })
      .catch((err) => {
  ...
  const handleStatusChange = (ids, newStatus) => {
    ...
    const token = localStorage.getItem("token");
    axios
      .put(`https://backndVoo.voo-hub.com/api/orgnization/changeEventVolunteerStatus/${ids}`, {
        status: newStatus,
      }, {
        headers: {
          Authorization: `Bearer ${token}`,
