 ...
 const Pendingusers = () => {
     const [data, setData] = useState([]);
      ...
      useEffect(() => {
        ...
        axios.get("https://backndVoo.voo-hub.com/api/orgnization/bnyadm", {
          headers: {
        ...
        const accept = (id, userName) => {
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                axios.put(`https://backndVoo.voo-hub.com/api/orgnization/bnyadm/accept/${id}`, {}, {
                  headers: {
                    Authorization: `Bearer ${token}`,
                  },
                })
          ...
          const reject = (id, userName) => {
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                axios.put(`https://backndVoo.voo-hub.com/api/orgnization/bnyadm/reject/${id}`, {}, {
                  headers: {
                    Authorization: `Bearer ${token}`,
                  },
                })
                     ...
                     {paginatedData.map((item, index) => (
                       <tr key={item.id} className='border-y hover:border-3 relative hover:bg-four'>
                         ...
                         <td className="w-[143px] h-[56px] lg:text-[12px] xl:text-[16px]  px-1">
                           <button className='underline ' onClick={() => navigate('/organizeation/pendingusersDetaklis', { state: { sendData: item.id } })}>
                             Details
                           </button>
