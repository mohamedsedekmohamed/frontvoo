...
const User = () => {
  ...
  const fetchUsers = () => {
    setIsLoading(true); // تشغيل التحميل
    setError(false);    // تصفير الخطأ
    ...
    axios.get("https://backndVoo.voo-hub.com/api/admin/users", {
      ...
      .catch(() => { 
        setError(true);      // إظهار صفحة الخطأ
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://backndVoo.voo-hub.com/api/admin/user/delete/${userId}`, {
          ...
          .catch(() => { Swal.fire('Error!', `There was an error while deleting ${userName}.`, 'error'); });
      }
    });
  };

  const handleEdit = (id) => navigate('/admin/addUser', { state: { sendData: id } });
  ...
  const labelMap = {
    Filter: "Filter", name: "User", email: "Email", age: "Age",
    phone: "Phone", "country.name": "Country", "city.name": "City",
  };
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      data.map((d, index) => ({
        SL: index + 1, Name: d.name || "-", Email: d.email || "-", Phone: d.phone || "-",
        City: d.city?.name || "-", country: d.country?.name || "-", total_hours: d.total_hours || "-",
        total_events: d.total_events || "-", total_tasks: d.total_tasks || "-", Account_status: d.account_status || "-",
      }))
    );
  ...
  const handleBulkDelete = () => {
    ...
    const token = localStorage.getItem("token");
    Swal.fire({
      title: "Are you sure?", text: "This action will delete selected users!",
  ...
  const handleBulkStatusChange = () => {
    if (selectedIds.length === 0) return;
    const token = localStorage.getItem("token");
    const newStatus = selectedIds.every((user) => user.status === "active") ? "inactive" : "active";
    axios.put("https://backndVoo.voo-hub.com/api/admin/user/statusGroup",
  ...
  const sortedData = useMemo(() => {
    ...
    return sortableData.sort((a, b) => {
      ...
      if (sortKey === 'age') return sortOrder === 'asc' ? aValue - bValue : bValue - aValue;
  ...
  const columns = [
    ...
    },
    {
      header: 'Details',
      render: (row) => (
        <button className='underline text-blue-600 hover:text-blue-800' onClick={() => navigate('/admin/userdetails', { state: { sendData: row.id } })}>
          Details
        </button>
      )
    },
    ...
    {
      header: 'Join Date',
      render: (row) => row?.created_at ? new Date(row.created_at).toISOString().split('T')[0] : "N/A"
    },
    {
      header: 'Status',
      render: (row) => (
        <label className="inline-flex items-center cursor-pointer">
          <input
            ...
            onChange={() => changestutes(row.id, row.account_status === 'active' ? 'inactive' : 'active')}
          />
          <div className="w-11 h-6 bg-gray-300 rounded-full peer peer-checked:bg-green-500 relative transition">
            <div className="absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition peer-checked:translate-x-5"></div>
          </div>
          <span className="ml-2 text-sm">{row.account_status === 'active' ? 'Active' : 'Inactive'}</span>
        </label>
      )
    },
    {
      header: (
        <input
          ...
          onChange={(e) => {
            if (e.target.checked) {
              setSelectedIds(paginatedData.map((item) => ({ id: item.id, status: item.account_status })));
            } else {
              setSelectedIds([]);
            }
          }}
      ...
      render: (row) => (
        <input
          ...
          onChange={(e) => {
            if (e.target.checked) {
              setSelectedIds(prev => [...prev, { id: row.id, status: row.account_status }]);
            } else {
              setSelectedIds(prev => prev.filter(user => user.id !== row.id));
            }
          }}
        />
      )
  ...
  return (
    <div>
      ...
      <div className='flex flex-wrap justify-between items-center gap-4 mb-6'>
        ...
        <button onClick={exportToExcel} className='p-2 h-10 bg-one text-white rounded-[10px] hover:bg-opacity-90 transition flex gap-2 items-center'>
          <span>Export</span>
          <FaDownload />
        </button>
        ...
        <div className='flex gap-2'>
          <div className='flex justify-center items-center bg-three py-1 px-2 rounded-[8px] gap-1 h-10'>
            <img src={filter} alt="filter" className='text-white w-4 h-4 md:w-6 md:h-6' />
            <select
          ...
          <button onClick={() => navigate('/admin/addUser')} className='flex justify-center items-center h-10 bg-white border-one border py-1 px-3 rounded-[8px] gap-1 hover:bg-gray-50 transition'>
      ...
      {selectedIds.length > 0 && (
        <div className="flex gap-2 mb-4">
          <button className="bg-one text-white px-4 py-2 rounded hover:bg-opacity-80 transition" onClick={handleBulkStatusChange}>
            Change Status
          </button>
          <button className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition" onClick={handleBulkDelete}>
            Delete Selected
          </button>
        </div>
      )}
