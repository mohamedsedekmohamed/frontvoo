...
const AddBenefitOr = ({ benfit, setbenfit, requirment, setrequirment }) => {
    const { t } = useTranslation();
  
  const handleAdd = (type) => {
    const newItem = { text: "", error: "" }; // بدون status
    if (type === "benefit") {
  ...
  const handleChange = (type, index, field, value) => {
    ...
    let error = "";
    if (field === "text" && !/^[^\d]*$/.test(value)) { // يمنع الأرقام
      error = "enter text";
    }
    updater[index][field] = value;
    updater[index].error = error;
    ...
    <div className="p-4 flex justify-evenly w-full">
      ...
      {benfit.map((item, idx) => (
          <div key={idx} className="flex gap-2 items-center mb-2">
            <input
              type="text"
              className={`border p-2 rounded w-full ${item.error && 'border-red-500'}`}
