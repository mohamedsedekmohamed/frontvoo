...
const AddAll = ({ navGo, name }) => {
  ...
  return (
    <div className="relative flex items-center justify-center w-full px-4 py-4 md:px-6">
      {/* Back Button (always left) */}
      <button
        onClick={() => navigate(navGo)}
        className="absolute left-4 md:left-6 p-2 rounded-lg hover:bg-gray-100 transition"
      >
        <IconDash
          width={24}
          height={24}
      ...
      {/* Title */}
      <h3 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold text-three text-center">
        {name}
      </h3>
    </div>
  );
